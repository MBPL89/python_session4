...
def fix_titles(titles):
    return [title.capitalize() for title in titles]

# Example
titles = print(fix_titles(titles))
...
#| autorun: false
#| min-lines: 3
def create_banner(word):
    return word.center(30, '*')

# Example
inp_word = input() 
print(create_banner(inp_word))
```
...
print(clean_left("***Important Notice"))  
...
def censor_text(text):
    ...
    for i in range(len(words)):
        stripped_word = words[i].rstrip(".,!?")  
        
        if stripped_word.lower() in bad_words:
            words[i] = words[i].replace(stripped_word, "****")
...
count_letter(message, "k")
count_letter(message, "m")
```
